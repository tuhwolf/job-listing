...
create_account = User.create([name: 'abc', email: 'abc@happy.com', password: '111111', password_confirmation: '111111', is_admin: 'false'])
puts 'Test account is created successfully!'

create_jobs = for i in 1..30 do
 job = jobs_info[rand(jobs_info.length)]
 company = companys_info[rand(companys_info.length)]
 Job.create!([title: job[0],description: job[1], wage_upper_bound: rand(50..99)*100,
   wage_lower_bound: rand(10..49)*100, is_hidden:"false",location:company[1],company:company[0]])
end
...
create_jobs = for i in 1..30 do
 ...
 company = companys_info[rand(companys_info.length)]
 Job.create!([title: job[0],description: job[1], wage_upper_bound: rand(50..99)*100,
   wage_lower_bound: rand(10..49)*100, is_hidden:"true",location:company[1],company:company[0]])
end
